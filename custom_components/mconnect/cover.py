...
async def async_setup_entry(
    ...
    def _add_new_covers() -> None:
        ...
        for device_id, device in coordinator.data.devices.items():
            ...
            elif dtype == "devices.types.SHUTTER":
                mode = get_shutter_mode(device)
                if mode == SHUTTER_MODE_RELAY:
                    # Mode 1 (relay): handled by switch.py + binary_sensor.py
                    known_ids.add(device_id)
                    continue

                value_id = _find_cover_value_id(device)
                if value_id:
                    tilt_vid: str | None = None
                    if mode == SHUTTER_MODE_VENETIAN:
                        tilt_vid = _find_blind_rotation_vid(device)
                    known_ids.add(device_id)
                    new_entities.append(
                        MConnectCover(
                            coordinator, device, value_id,
                            tilt_value_id=tilt_vid,
                        )
                    )
            else:
                # Standard covers (DOOR, GARAGE, WINDOW)
                value_id = _find_cover_value_id(device)
                if value_id:
                    known_ids.add(device_id)
...
def _find_cover_value_id(device: dict[str, Any]) -> str | None:
    """Find the best value_id to use as the cover control."""
    for v in device.get("values", []):
        vtype = v.get("type", "")
        vid = v.get("value_id", "")
        if vid in SHUTTER_CONFIG_VALUE_IDS:
            continue
...
def _find_blind_rotation_vid(device: dict[str, Any]) -> str | None:
    """Find the blind_rotation value_id for venetian mode."""
    for v in device.get("values", []):
        if v.get("value_id") == "blind_rotation":
            return "blind_rotation"
    return None
...
class MConnectCover(MConnectEntity, CoverEntity):
    ...
    def __init__(
        ...
        self._tilt_value_id = tilt_value_id
        ...
        if self._is_link:
            ...
            only_open_close = False
            if val_obj:
                attrs = val_obj.get("attributes") or {}
                only_open_close = attrs.get("onlyOpenClose", False)
            if val_type in (VALUE_TYPE_MULTILEVEL, VALUE_TYPE_OPEN_CLOSE):
                if not only_open_close:
                    features |= CoverEntityFeature.SET_POSITION
                features |= CoverEntityFeature.STOP

        # Tilt support for venetian blinds (mode 2)
        if self._tilt_value_id:
        ...
        # Stop value_id for non-LINK devices
    ...
    def current_cover_tilt_position(self) -> int | None:
        """Return current tilt for venetian blinds (0=closed, 100=open)."""
        if not self._tilt_value_id:
            return None
        val_obj = self._data.get_device_value_obj(
            self._device_id, self._tilt_value_id
        )
        if not val_obj:
            return None
        try:
    ...
    # ── Tilt commands (venetian blinds, mode 2) ──────────────────────────

    async def async_open_cover_tilt(self, **kwargs: Any) -> None:
        """Open tilt to fully open position."""
        if self._tilt_value_id:
            val_obj = self._data.get_device_value_obj(
                self._device_id, self._tilt_value_id
            )
            v_max = int(val_obj.get("max", 100)) if val_obj else 100
            await self._send_value(self._tilt_value_id, v_max)

    async def async_close_cover_tilt(self, **kwargs: Any) -> None:
        """Close tilt to fully closed position."""
        if self._tilt_value_id:
            val_obj = self._data.get_device_value_obj(
                self._device_id, self._tilt_value_id
            )
            v_min = int(val_obj.get("min", 0)) if val_obj else 0
            await self._send_value(self._tilt_value_id, v_min)

    async def async_set_cover_tilt_position(self, **kwargs: Any) -> None:
        """Set tilt to a specific position (0=closed, 100=open)."""
        if not self._tilt_value_id:
            return
        ...
        if val_obj:
            v_min = int(val_obj.get("min", 0))
            v_max = int(val_obj.get("max", 100))
            actual = round(v_min + (tilt / 100) * (v_max - v_min))
            await self._send_value(self._tilt_value_id, actual)
        else:
            await self._send_value(self._tilt_value_id, tilt)
